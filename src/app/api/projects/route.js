...
const projectFilePath = path.join(process.cwd(), "public/mocks/projects.json");

export async function GET() {
  try {
    const projects = await fsPromises.readFile(projectFilePath, "utf-8");
    console.log("Projects: ", projects);
    const json = JSON.parse(projects);
    return NextResponse.json(json);
  } catch (error) {
    return new NextResponse(JSON.stringify({ message: "No projects found!" }), {
      status: 404,
...
//     const jsonArray = JSON.parse(projects);
...
// //     desiredDish.description = description
// //       ? description
// //       : desiredDish.description;
